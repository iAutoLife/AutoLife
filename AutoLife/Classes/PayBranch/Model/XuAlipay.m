...
+(void) alipayWithDic:(NSDictionary *)dictionary andFinish:(void(^)(NSDictionary *resultDic))finished{
    ...
    order.seller = seller;
    order.tradeNO = [self out_trade_no:NO]; //订单ID（由商家自行制定）
    order.productName = dictionary[@"title"]; //商品标题
    order.productDescription = dictionary[@"parkingLot"]; //商品描述
...
+ (NSString *)out_trade_no:(BOOL)isParking
{
    NSMutableString *resultStr = [[NSMutableString alloc] init];
    [resultStr appendString:(isParking ? @"527" : @"505")];
    [resultStr appendString:@"20160624"];
    [resultStr appendString:@"20"];
    [resultStr appendString:@"16062413"];
