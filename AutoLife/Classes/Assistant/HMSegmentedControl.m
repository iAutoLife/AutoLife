...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (self.isXuStyle) {
                CGRect newRect = CGRectMake(rect.origin.x, rect.origin.y - 3, rect.size.width, rect.size.height + 6);
                ...
                if (self.selectedSegmentIndex == idx) {
                    titleLayer.backgroundColor = [[UIColor colorWithRed:0.98f green:0.98f blue:0.98f alpha:1.0f] CGColor];
                } else {
